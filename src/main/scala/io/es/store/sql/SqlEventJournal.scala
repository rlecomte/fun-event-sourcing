...
import io.es.infra.data.{Aggregate, Event, _}

case class SqlEventJournal(xa: Transactor[IO]) extends EventJournal[Json] with DoobieMetaInstances {
  ...
  override def write[E <: Event](encoder: EventEncoder[E, Json], decoder: EventDecoder[E, Json], aggregateTag: String)
    ...
    val insertNewAggregate: ConnectionIO[Unit] = {
      val sql: Update0 =
        sql"""
               INSERT INTO aggregates (id, aggregate_type, version) VALUES ($aggregateId, $aggregateTag, 0)
            """
          .update
      sql.run.map(_ => ())
    }
    ...
    val query = for {
      ...
      _ <- {
        ...
        val eventsWithVersion = events.toList.zip(incVersions)
          .map { case (e, v) => encoder(aggregateId, aggregateTag, v, ZonedDateTime.now(), e) }

        Update[(AggregateId, Version, Json, String)](insertEvents)
  ...
  override def hydrate[A <: Aggregate, E <: Event](decoder: EventDecoder[E, Json], handler: EventHandler[A, E], aggregateId: AggregateId): IO[Option[(A, Version)]] = {
    ...
    sql.process
      .map { rawEvent => (decoder(rawEvent), rawEvent.version) }
      .fold[Option[(A, Version)]](None) { case (s, (e, v)) => Some((handler(s.map(_._1), e), v)) }
      .list
      .transact(xa)
