...
object JsonEvent {
  ...
  trait JsonEncoderDecoderBuilder[E <: Event] {
    def eventDecoder[T <: String, A <: Aggregate](implicit agg: AggregateTag[A, _, E], decoder: Decoder[E]): EventDecoder[E, Json] = new PartialFunction[RawEvent[Json], E] {
    ...
    def eventEncoder[T <: String, A <: Aggregate](implicit agg: AggregateTag[A, _, E], encoder: Encoder[E]): EventEncoder[E, Json] = (aggId, version, date, event) => {
