...
package object turtle {
  ...
  case class Position(x: Int, y: Int) {
    def move(dir: Direction, distance: Int): Position =
      dir match {
        case North => copy(y = y + distance);
        case South => copy(y = y - distance)
  ...
  object Position {
    ...
    def move(pos: Position, dir: Direction, distance: Int): Position =
      dir match {
        case North => pos.copy(y = pos.y + distance);
        case South => pos.copy(y = pos.y - distance)
        case Est   => pos.copy(x = pos.x + distance);
        case West  => pos.copy(x = pos.x - distance)
      }
  }
}
