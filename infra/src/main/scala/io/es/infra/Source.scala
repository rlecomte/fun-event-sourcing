...
case class Source[S, E](
  ...
  def andThen(f: PartialSource[S, E]): Source[S, E] =
    Source(root, list.andThen(f))
}
...
case class PartialSource[S, E](list: List[S => Result[E]] = Nil) extends AnyVal {
  ...
  def andThen(f: PartialSource[S, E]): PartialSource[S, E] =
    PartialSource(f.list ++ list)
}

object Source {
  def create[S, E](init: Result[E]): Source[S, E] = Source(NewSource(init))

  def hydrate[S, E](id: java.util.UUID): Source[S, E] =
    Source(HydratedSource(id))

  def partial[S, E](f: S => Result[E]): PartialSource[S, E] =
    PartialSource(f :: Nil)
